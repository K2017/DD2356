...
int main(int argc, char* argv[]){
	...
	printf("DFTW calculation with N = %d, number of threads = %d\n",N, threads);
	...
	for (int r = 0; r < R; ++r) {
		...
		for (int i = 0; i < G; ++i) {
			...
			setOutputZero(xr_check,xi_check,N);
			setOutputZero(Xr_o,Xi_o,N);
			double reset_time = omp_get_wtime() - start_reset;
	...
	double avg_time = total_time / R;
	double ss = 0.0;
	for (int r = 0; r < R; ++r) {
		ss += (times[r] - avg_time) * (times[r] - avg_time);
	}
	double std_dev = sqrt(ss / R);
	printf("DFTW computation in average of %f seconds\n",avg_time);
...
int DFT(int idft, double* xr, double* xi, double* Xr_o, double* Xi_o, int N){
	for (int k=0 ; k<N ; k++)
	{
		...
		#pragma omp parallel
		{
			...
			for (int n=0 ; n<N ; n++)  {
				// Real part of X[k]
				real += xr[n] * cos(n * k * PI2 / N) + idft*xi[n]*sin(n * k * PI2 / N);
				// Imaginary part of X[k]
				imag += -idft*xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
			} 
